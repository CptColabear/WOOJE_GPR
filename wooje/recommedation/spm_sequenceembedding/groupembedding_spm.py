...
user_tl_emb_col = db["user_tl_pattern_embeddings_attn_v2_spm70"]
user_cluster_vec_col = db["4. user_cluster_embedding_bestk"]
group_emb_col = db["9. group_embeddings_v1_spm"]
...
def build_group_embedding(users, TL, cluster_id_k, tl_embedding_cache, cluster_score_cache, beta=10.0):
    ...
    if score_sum <= 0:
        # 모든 친숙도 점수가 0이면 단순 평균으로 대체
        weights = np.ones_like(raw_scores) / len(raw_scores)
    else:
        # 친숙도 점수를 비율 가중치로 변환
        weights = raw_scores / score_sum

    group_emb = np.sum(user_embeddings * weights[:, None], axis=0)
    group_emb = l2_normalize(group_emb)
...
if sample_out is None:
    ...
    if "cluster_weights" in sample_out:
        print("cluster_weights:", sample_out["cluster_weights"])
