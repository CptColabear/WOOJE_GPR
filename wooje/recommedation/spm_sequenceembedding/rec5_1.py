...
import wooje.recommedation.spm_sequenceembedding.rec5 as base_exp
...
def configure_split():
    ...
    print("\n" + "=" * 100)
    print("[rec2_1] Using split ratio train/valid/test = 70/20/10")
    print(f"[rec2_1] total usable groups = {len(base_exp.usable_group_docs)}")
    print(f"[rec2_1] train size = {len(base_exp.train_docs)}")
    print(f"[rec2_1] valid size = {len(base_exp.valid_docs)}")
    print(f"[rec2_1] test size  = {len(base_exp.test_docs)}")
    print("=" * 100)
...
def main():
    ...
    print(f"[rec2_1] saved results to: {RESULT_JSON_PATH.name}")
