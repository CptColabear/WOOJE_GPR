...
import wooje.jp.recommedation.rec2copy_jp as base_exp
...
RESULT_JSON_PATH = Path(__file__).with_name("topk_results_summary_70_20_10_jp.json")
...
def configure_split():
    ...
    print("\n" + "=" * 100)
    print("[rec2_2_jp] Using split ratio train/valid/test = 70/20/10 (Tokyo / ejoow2)")
    print(f"[rec2_2_jp] total usable groups = {len(base_exp.usable_group_docs)}")
    print(f"[rec2_2_jp] train size = {len(base_exp.train_docs)}")
    print(f"[rec2_2_jp] valid size = {len(base_exp.valid_docs)}")
    print(f"[rec2_2_jp] test size  = {len(base_exp.test_docs)}")
    print("=" * 100)
...
def main():
    ...
    baseline_model, baseline_valid_topk, baseline_test_topk = base_exp.run_experiment(
        exp_name="baseline_cluster_negative_70_20_10_jp",
        sampling_mode="baseline",
    )

    base_exp.print_metric_table("1) baseline cluster negative - VALID (70/20/10, Tokyo)", baseline_valid_topk)
    base_exp.print_metric_table("1) baseline cluster negative - TEST (70/20/10, Tokyo)", baseline_test_topk)

    hard_model, hard_valid_topk, hard_test_topk = base_exp.run_experiment(
        exp_name="hard_cluster_distance_negative_70_20_10_jp",
        sampling_mode="hard",
    )

    base_exp.print_metric_table("2) hard cluster distance negative - VALID (70/20/10, Tokyo)", hard_valid_topk)
    base_exp.print_metric_table("2) hard cluster distance negative - TEST (70/20/10, Tokyo)", hard_test_topk)

    base_exp.compare_results(
        title="TEST performance comparison (70/20/10, Tokyo): baseline vs hard negative",
    ...
    print(f"[rec2_2_jp] saved results to: {RESULT_JSON_PATH.name}")
