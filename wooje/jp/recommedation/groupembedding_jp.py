"""Stage 9: group embedding for JP -> "99. group_embeddings_v2".

Direct port of groupembedding.py (confirmed via full read of the original)
-- only DB_NAME and the group_checkin_col name change; everything else
(confidence-weighted averaging of member pattern embeddings, L2
normalization, BETA, batching) is unchanged and country-agnostic.
"""
import math
...
def load_user_tl_embedding_cache(user_tl_keys, query_batch_size=500):
    ...
    for key_batch in batched(key_list, query_batch_size):
        query = {"$or": [{"user_id": user_id, "TL": tl} for user_id, tl in key_batch]}
        cursor = user_tl_emb_col.find(query, {"_id": 0, "user_id": 1, "TL": 1, "pattern_embedding": 1})
        for doc in cursor:
            cache[(str(doc["user_id"]), int(doc["TL"]))] = np.array(doc["pattern_embedding"], dtype=np.float32)
    return cache
...
def load_user_cluster_score_cache(user_ids):
    ...
    if not user_id_list:
        return cache
    cursor = user_cluster_vec_col.find({"user_id": {"$in": user_id_list}}, {"_id": 0, "user_id": 1, "embedding": 1})
    for doc in cursor:
...
def build_group_embedding(users, TL, cluster_id_k, tl_embedding_cache, cluster_score_cache, beta=10.0):
    ...
    weights = np.ones_like(raw_scores) / len(raw_scores) if score_sum <= 0 else raw_scores / score_sum
    group_emb = l2_normalize(np.sum(user_embeddings * weights[:, None], axis=0))
...
projection = {
    "_id": 1, "Group_ID": 1, "Users": 1, "TL": 1, "cluster_id_k": 1,
    "Latitude": 1, "Longitude": 1, "VenueID": 1, "VenueCategoryname": 1, "users": 1,
}

written = processed = skipped = 0
...
for doc_batch in tqdm(batched(cursor, GROUP_BATCH_SIZE), total=math.ceil(total_docs / GROUP_BATCH_SIZE),
                       desc="Building group embeddings (batch)"):
    user_tl_keys = {(str(uid), int(doc["TL"])) for doc in doc_batch for uid in doc["Users"]}
    user_ids = {str(uid) for doc in doc_batch for uid in doc["Users"]}
    ...
    for doc in doc_batch:
        ...
        result = build_group_embedding(users, TL, cluster_id_k, tl_embedding_cache, cluster_score_cache, beta=BETA)
        if result is None:
            skipped += 1
            continue
        ...
        ops.append(UpdateOne({"_id": source_id}, {"$set": out_doc}, upsert=True))
    ...
    print(f"processed={processed:,} written={written:,} skipped={skipped:,}")
